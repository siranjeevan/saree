...
const WishlistCard = ({ saree, index, onViewDetails }) => {
      ...
      <Card className="overflow-hidden bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-2xl transition-all duration-300">
        <CardHeader className="p-0 relative">
          <div className="relative overflow-hidden bg-gradient-to-br from-purple-50 to-pink-50">
            <img
              src={saree.ImageLink || `https://picsum.photos/400/600?random=${index + 1}`}
              alt={saree.SareeName}
              className="w-full h-64 object-cover"
              loading="lazy"
              onError={(e) => {
                e.target.src = `https://via.placeholder.com/400x600/8B5CF6/FFFFFF?text=${encodeURIComponent(saree.SareeName)}`
              }}
            />
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.9 }}
              onClick={() => removeFromWishlist(saree.SareeName)}
            >
              <X className="h-4 w-4" />
            </motion.button>
